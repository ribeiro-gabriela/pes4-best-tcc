...
int tftpClientGet(const char* ip_addr, lurFilesDescriptionHeader_t fileInfo)
{
    ...
    *(uint16_t*)packet = htons(TFTP_OP_RRQ);
    ...
    ESP_LOGI(TAG, "rrq sent to %s: %s", ip_addr, fileInfo.loadPartNumberName);
    ...
    char fPath[MAX_FILE_PATH_LEN];
    snprintf(fPath, MAX_FILE_PATH_LEN, "/spiffs/%s", fileInfo.loadPartNumberName);
    ...
    while (1)
    {
        ...
        int len = recvfrom(sock, packet, sizeof(packet), 0, (struct sockaddr*)&sender, &slen);
        ...
        if (len > 4)
        {
            ...
            if (opcode == TFTP_OP_DATA && block == expected_block)
            {
                ...
                if (written < data_len)
                {
                    ESP_LOGE(TAG, "could not write data on partition. aborting operation ...");

                    char* abortInfo = "could not write data on partition";
                    sendStatusToClient(total_received,
                                       ARINC_OP_ABORTED_BY_TARGET,
                                       strlen(abortInfo),
                                       abortInfo,
                                       block,
                ...
                char* statusDesc = "operation in progress";

                if (block % 10 == 0)
                {
                    sendStatusToClient(total_received,
                                       ARINC_OPERATION_IN_PROGRESS,
                                       strlen(statusDesc),
                                       statusDesc,
                                       block,
                                       fileInfo.headerFileNameLength,
                ...
                if (data_len < 512)
                {
                    ESP_LOGI(TAG, "download finished. received: %d bytes", total_received);

                    QueueMessage_t eventMessage;
                    eventMessage.eventID = COMM_TRANSFER_COMPLETE;
                    sprintf((char*)eventMessage.logMessage,
                            "received image. %u bytes received",
                            (unsigned)total_received);

                    if (xQueueSend(BCQueue, (void*)&eventMessage, portMAX_DELAY) != pdPASS)
                    {
                    ...
                    strncpy(imageFilename, fileInfo.loadPartNumberName, sizeof(imageFilename));

                    char* finishedDesc = "operation finished without error";
                    sendStatusToClient(total_received,
                                       ARINC_OP_COMPLETED_WITHOUT_ERROR,
                                       strlen(finishedDesc),
                                       finishedDesc,
                                       block,
                                       fileInfo.headerFileNameLength,
        ...
        else
        {
            ...
            {
                sendto(
                    sock, packet, req_len, 0, (struct sockaddr*)&server_addr, sizeof(server_addr));
            }
            else
            {
                ...
                sendto(sock,
                       ...
                       sizeof(server_addr)); // Cuidado com o endereço aqui (deve ser o da sessão)
            }
        }
    }
...
int tftpClientPut(const char* ip_addr, const char* filename, uint8_t* payload, size_t payload_size)
{
    ...
    *(uint16_t*)packet = htons(TFTP_OP_WRQ); // Opcode WRQ
    ...
    while (1)
    {
        int len = recvfrom(sock, packet, sizeof(packet), 0, (struct sockaddr*)&session_addr, &slen);
        ...
        if (len > 3)
        {
            ...
            if (opcode == TFTP_OP_ACK && block == current_block)
            {
                ...
                if (payload_offset >= payload_size)
                {
                    ...
                    if (payload_size % TFTP_DATA_SIZE == 0 && payload_size > 0)
                    {
                        ...
                        *(uint16_t*)(data_packet + 2) = htons(current_block);
                        ...
                        // A transferência terminará no próximo loop após receber o ACK para este
                        // bloco.
                        continue;
                    }
                    // Se o ACK for para o pacote vazio final, já deve ter sido pego pelo 'break'
                    // acima. Se chegar aqui, pode ser um erro de lógica, mas por segurança, não
                    // envia mais nada.
                ...
                size_t data_len =
                    (remaining_bytes > TFTP_DATA_SIZE) ? TFTP_DATA_SIZE : remaining_bytes;
                ...
                *(uint16_t*)(data_packet + 2) = htons(current_block); // Número do Bloco
                ...
                ESP_LOGI(TAG,
                         "Sent block %d (%zu bytes). Total sent: %zu",
                         current_block,
                         data_len,
                         total_sent);
            }
            ...
            else
            {
                ...
                ESP_LOGW(TAG,
                         "Received unexpected opcode %d or block %d (expected %d)",
                         opcode,
                         block,
                         current_block);
            }
        ...
        else
        {
            ...
            else
            {
                ...
                // Cabeçalho do pacote DATA
                *(uint16_t*)data_packet = htons(TFTP_OP_DATA); // Opcode DATA
                *(uint16_t*)(data_packet + 2) =
                    htons(current_block); // Número do Bloco (O bloco que está sendo esperado o ACK)
                ...
                ESP_LOGW(TAG,
                         "Retransmitted block %d (%zu bytes) from offset %zu",
                         current_block,
                         data_len,
                         retransmit_offset);
            }
...
void taskArincSendLus(void* params)
{
    ...
    for (;;)
    {
        bytesReceived = xMessageBufferReceive(
            messageBufferHandle, (void*)receivedBuffer, MESSAGE_BUFFER_SIZE, portMAX_DELAY);
...
{
    messageBufferHandle =
        xMessageBufferCreateStatic(MESSAGE_BUFFER_SIZE, messageBuffer, &messageBufferStruct);
    ...
    arincSendLusTaskHandle = xTaskCreateStaticPinnedToCore(taskArincSendLus,
                                                           "send LUS task",
...
void deinitTaskSendLus()
{
    ...
    ESP_LOGI(TAG, "deinitializing lus sender task");
}
