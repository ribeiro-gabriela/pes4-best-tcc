...
static int
sendAck(int sock, struct sockaddr_storage* destAddr, socklen_t addrLen, uint16_t blockNumber)
{
    uint8_t packet[4];
...
void tftpSessionTask(void* pvParameters)
{
            ...
            if (len > 0)
            {
                ...
                if (recv_opcode == TFTP_OP_DATA)
                {
                    ...
                    if (recv_block == expected_block)
                    {
                        ...
                        arincErr_t writeRes = arinc_append_data(data_ptr, data_len);
                        ...
                        if (data_len < 512)
                        {
                            ...
                            if (isFileValid == ARINC_ERR_OK)
                            {
                                ...
                                if (xQueueSend(BCQueue, (void*)&eventMessage, portMAX_DELAY) !=
                                    pdPASS)
                                {
                            ...
                            arincErr_t res = readLoadUploadingRequest(
                                final_size, final_ptr, filesList, &parsedCount);

                            if (res == ARINC_ERR_OK)
                            {
                                ESP_LOGI(TAG,
                                         "LUR file parsed succesfully  %d "
                                         "files found.",
                                         parsedCount);

                                tftpClientGet("192.168.4.2",
                                              (lurFilesDescriptionHeader_t)filesList[0]);
                            }
                            else
                            {
                                ESP_LOGE(TAG, "LUR file corrupted or invalid. %d", res);
                            }
    ...
    {
        ARINC_DATA_TYPE_t filetype = ARINC_NONE;
        arincErr_t isFileValid = arincValidateFileAndGetName(config->filename, filetype);
        ...
        if (isFileValid == ARINC_ERR_OK)
        {
            ESP_LOGW(TAG, "new LUI file received");
